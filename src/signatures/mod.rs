...
fn is_xls<T: Seek + Read>(buffer: &mut ByteReader<T>) -> Result<bool> {
  ...
  if *b_pattern1
    == [
      0x57, 0x00, 0x6f, 0x00, 0x72, 0x00, 0x6b, 0x00, 0x62, 0x00, 0x6f, 0x00, 0x6f, 0x00, 0x6b,
      0x00,
    ] {
    return Ok(true);
  }
...
fn is_ppt<T: Seek + Read>(buffer: &mut ByteReader<T>) -> Result<bool> {
  ...
  if *b_pattern1
    == [
      0x50, 0x00, 0x6f, 0x00, 0x77, 0x00, 0x65, 0x00, 0x72, 0x00, 0x50, 0x00, 0x6f, 0x00, 0x69,
      0x00, 0x6e, 0x00, 0x74,
    ] {
    buffer.take_raw_bytes_offset(b_pattern1, baddr + 19)?;
    if *b_pattern1
      == [
        0x00, 0x20, 0x00, 0x44, 0x00, 0x6f, 0x00, 0x63, 0x00, 0x75, 0x00, 0x6d, 0x00, 0x65, 0x00,
        0x6e, 0x00, 0x74, 0x00,
      ] {
      return Ok(true);
    }
  ...
  if *b_pattern2
    == [
      0x43, 0x00, 0x75, 0x00, 0x72, 0x00, 0x72, 0x00, 0x65, 0x00, 0x6e, 0x00, 0x74, 0x00, 0x20,
      0x00, 0x55, 0x00, 0x73, 0x00, 0x65, 0x00, 0x72, 0x00,
    ] {
    return Ok(true);
  }
  ...
  if *b_pattern3
    == [
      0x50, 0x00, 0x69, 0x00, 0x63, 0x00, 0x74, 0x00, 0x75, 0x00, 0x72, 0x00, 0x65, 0x00, 0x73,
      0x00,
    ] {
    return Ok(true);
  }
...
pub fn analyze_zip<S: AsRef<str>>(t: S) -> Result<FileType> {
  let f = File::open(t.as_ref())?;
...
pub fn detect_filetype<S: AsRef<str>>(t: &S) -> Result<FileType> {
  let f = File::open(t.as_ref())?;
  detect_type(f).and_then(|ft| match ft {
    FileType::Zip => analyze_zip(t.as_ref()),
    _ => Ok(ft),
  })
}
